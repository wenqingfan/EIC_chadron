...
class hadron_gen
{
  private:
    ...
    TH2D* h2d_hadron_nu_vs_eta_gen[Q2bin][xbin];
  ...
  public:
    hadron_gen(int _hadron_id)
    {
      ...
      for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
      {
        ...
        {
          h2d_hadron_pt_vs_eta_gen[iQ2][ix] = new TH2D(Form("h2d_hadron_%d_pt_vs_eta_gen_%d_%d",hadron_id,iQ2,ix),"D0 pt vs eta",100,0,10,160,-10,10);
          h2d_hadron_pt_vs_eta_gen[iQ2][ix]->Sumw2();

          h2d_hadron_z_vs_eta_gen[iQ2][ix] = new TH2D(Form("h2d_hadron_%d_z_vs_eta_gen_%d_%d",hadron_id,iQ2,ix),"D0 z vs eta",100,0,1,160,-10,10);
          h2d_hadron_z_vs_eta_gen[iQ2][ix]->Sumw2();

          h2d_hadron_nu_vs_eta_gen[iQ2][ix] = new TH2D(Form("h2d_hadron_%d_nu_vs_eta_gen_%d_%d",hadron_id,iQ2,ix),"D0 z vs eta",150,0,3000,160,-10,10);
          h2d_hadron_nu_vs_eta_gen[iQ2][ix]->Sumw2();
        }
      }
    }
    virtual ~hadron_gen()
    {
      ...
      for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
      {
        for (int ix = 0; ix < xbin; ++ix)
        {
          ...
          delete h2d_hadron_nu_vs_eta_gen[iQ2][ix];
        }
      }
    };
    ...
    void Reset()
    {
      ...
      for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
      {
        for (int ix = 0; ix < xbin; ++ix)
        {
          ...
          h2d_hadron_nu_vs_eta_gen[iQ2][ix]->Reset("ICESM");
        }
      }
    }
    ...
    void FillGenKin(erhic::EventPythia* py_evt)
    {
      ...
      for(int ipart = 0; ipart < py_evt->GetNTracks(); ipart++)
      {
        ...
        if (iQ2bin>=0 && ixbin>=0)
        {
          ...
          h2d_hadron_nu_vs_eta_gen[iQ2bin][ixbin]->Fill(nu_true,hadron_mom4_gen.PseudoRapidity());
          ...
          h2d_hadron_nu_vs_eta_gen[iQ2bin][xbin-1]->Fill(nu_true,hadron_mom4_gen.PseudoRapidity());
          ...
          h2d_hadron_nu_vs_eta_gen[Q2bin-1][ixbin]->Fill(nu_true,hadron_mom4_gen.PseudoRapidity());
          ...
          h2d_hadron_nu_vs_eta_gen[Q2bin-1][xbin-1]->Fill(nu_true,hadron_mom4_gen.PseudoRapidity());
        } 
      }
    }
    ...
    void Write()
    {
      ...
      for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
      {
        for (int ix = 0; ix < xbin; ++ix)
        {
          ...
          h2d_hadron_nu_vs_eta_gen[iQ2][ix]->Write();
        }
      }
    }
};
...
{ 
  ...
  TH1D* h1d_nevt_w_charm[Q2bin][xbin] = {0};
  for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
  {
    for (int ix = 0; ix < xbin; ++ix)
    {
      ...
      h1d_nevt_w_charm[iQ2][ix] = new TH1D(Form("h1d_nevt_w_charm_%d_%d",iQ2,ix),"event counter",1,0.5,1.5);
      h1d_nevt_w_charm[iQ2][ix]->Sumw2(); 
    }
  }
  ...
  {    
    ...
    // HERMES selections
    if (event->GetTrueY()>0.85) continue;
    if (event->GetTrueW2()<4) continue;
    if (event->GetTrueNu()<6) continue;
    ...
    if (iQ2bin>=0 && ixbin>=0)
    {
      ...
      {
        h1d_nevt_w_charm[iQ2bin][ixbin]->Fill(1);
        h1d_nevt_w_charm[iQ2bin][xbin-1]->Fill(1);
        h1d_nevt_w_charm[Q2bin-1][ixbin]->Fill(1);
        h1d_nevt_w_charm[Q2bin-1][xbin-1]->Fill(1);
      }
    } 
